defmodule Magma.Prompt.Template do
  ...
  def custom_prompt_obsidian_template(project, prompt) do
    ...
    aliases: []
    ---
    #{prompt_body(prompt, "{{NAME}}", project)}
    """
  end

  def render(%Session{} = session, project), do: Session.Template.render(session, project)

  def render(%Prompt{} = prompt, project) do
    prompt_body(prompt, Prompt.title(prompt), project)
  end
  ...
  defp prompt_body(prompt, title, project) do
    """
    #{controls(prompt)}
